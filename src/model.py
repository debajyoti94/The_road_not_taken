...
class LanguageModel:
    def train_model(
        ...
        one_hot_targets = np.zeros(
            (len(input_sequences), max_sequence_len, actual_vocab_size)
        )
        ...
        embedding_layer = Embedding(
            actual_vocab_size, config.EMBEDDING_DIM, weights=[embedding_matrix]
        )
        ...
        encoder_model.compile(
            loss=config.LOSS_FN, optimizer=Adam(lr=config.LR), metrics=["accuracy"]
        )
        ...
        # given an input word, we will get a
        # probability distribution of the output words
        ...
        sampling_model = Model(
            [input_decoder, initial_h, initial_c], [output_layer_decoder, h, c]
        )
    ...
    def sample_text_gen(self, sampling_model, word2idx, idx2word):
        ...
        :param sampling_model: the decoder
