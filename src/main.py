...
# introducing this gate so that
# the code below this gate cannot be imported
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train",
        type=str,
        help='Provide arguments "--train seq2seq"'
    ...
    parser.add_argument(
        "--generate",
        type=str,
        help='Provide arguments "--generate text"'
             ' to generate text using the seq2seq model.',
    )
    ...
    if args.preprocess == "data":
        ...
        (
            ...
            tokenizer,
            max_sequence_len,
        ) = dp_obj.tokenize_input(config.RAW_DATASET)
        ...
        embedding_matrix, num_words = dp_obj.create_embedding_matrix(
            tokenizer, word_vector_dict
        )
        ...
        # we will load all the necessary files
        ...
        encoder_model, sampling_model = lm.train_model(
            input_sequences[0],
            output_sequences[0],
            embedding_matrix[0],
        ...
        lm = model.LanguageModel()
        # generating 5 lines at a time
        while True:
            for _ in range(5):
                print(
                    lm.sample_text_gen(
                        sampling_model,
                        tokenizer[0].word_index,
