...
class DataPreprocessing(MustHaveDP):
    ...
    def tokenize_input(self, dataset_path):
        """
         tokenize the input/output
        :param dataset_path: file path to input dataset
        ...
        max_sequence_len = min(max_sequence_len, config.MAX_SEQUENCE_LENGTH)
        ...
        return input_sequences, output_sequences, tokenizer, max_sequence_len
    ...
    def create_embedding_matrix(self, tokenizer, word_vector_dict):
        ...
        :param tokenizer: keras tokenizer used to fit texts
        :param word_vector_dict:
        :return: embedding matrix, num_words(actual_vocab_size)
        """

        word2idx = tokenizer.word_index
        num_words = min(
            config.MAX_VOCAB_SIZE, len(word2idx) + 1
        )  # +1 bcoz the index in tokenizer starts at 1
        ...
        # print(embedding_matrix[word2idx["<sos>"]],
        # embedding_matrix[word2idx["<eos>"]])
...
class DumpLoadFile:
    def load_file(self, filename):
        ...
        :param filename: path to the file that you want to load
        :return: loaded file
        """
    ...
    def dump_file(self, filename, *file):
        """
        Dumping pickled files
        :param filename: filename + path
        :param file: the files that you want to dump.
        :return: nothing
        """
